...
int	handle_threads(t_data *data)
{
	...
	pthread_t   death_monitor_threads[data->n_philos];
	...
	while (i < data->n_philos)
	{
		if (pthread_create(&data->id[i], NULL, &routine, &data->philo[i]))
			return (THREAD_ERR);
		if (pthread_create(&death_monitor_threads[i], NULL, &death_monitor, &data->philo[i]))
			return (THREAD_ERR);
		i++;
	}
	i = 0;
...
void eat(t_philo *philo)
{
	...
	philo->n_eat_times++;
	if (philo->data->n_meals > 0 && philo->n_eat_times >= philo->data->n_meals)
	{
		philo->data->finished++;
	}
...
void *death_monitor(void *data_pointer)
{
	...
	{
		pthread_mutex_lock(&philo->lock);
		if (!philo->eating && get_time() > philo->death_time)
		{
